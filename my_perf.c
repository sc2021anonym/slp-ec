...
pair one_step(int m /* 14 */, int k /* 10 */, int nerrs /* 4 */,
	      u8 *a, u8 *g_tbls,
	      u8 *src_in_err, u8 *src_err_list)
{
  pair p = { 0.0, 0.0 };
...
int main(int argc, char *argv[])
{
  ...
  for(int iter = 0; iter < count; ++iter) {
    pair p = one_step(m, k, nerrs, a, g_tbls, src_in_err, src_err_list);
    enc[iter] = p.v1;
    dec[iter] = p.v2;
  }
