...
pub unsafe fn page_generic_slow(dst: *mut u8, vs: &[*const u8]) {
    ...
    if vs.contains(&(dst as *const u8)) {
        ...
        std::ptr::copy_nonoverlapping(vs[0], dst, 4096);
...
mod test {
    ...
    #[test]
    fn avx2xor2_test1() {
        let left = gen_data(4096);
        let right = gen_data(4096);

        let mut dst1 = vec![0; 4096];
        naiive_xor(&mut dst1, &left, &right);

        let mut dst2 = vec![0; 4096];
        unsafe {
            avx2_page_xor2(dst2.as_mut_ptr(), left.as_ptr(), right.as_ptr());
        }

        let mut dst3 = vec![0; 4096];
        unsafe {
    ...
    #[test]
    fn avx2xor3_test1() {
        let v1 = gen_data(4096);
        let v2 = gen_data(4096);
        let v3 = gen_data(4096);
    ...
    fn generic_slow_test() {
        ...
        let v14 = gen_data(4096);
        let v15 = gen_data(4096);
        let v16 = gen_data(4096);
        let v17 = gen_data(4096);
